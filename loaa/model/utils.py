...
def generate_loaa_buffers(loaa_choices, device="cuda"):
    """
    Generate buffers for the loaa structure based on the provided choices.
    
    Parameters:
    - loaa_choices (list): A nested list representing tree in the loaa structure.
    - device (str): Device to which the tensors should be moved. Default is "cuda".
    
    Returns:
    - dict: A dictionary containing buffers related to the loaa structure.
    """

    # Sort the loaa_choices based on their lengths and then their values
    sorted_loaa_choices = sorted(loaa_choices, key=lambda x: (len(x), x))
    loaa_len = len(sorted_loaa_choices) + 1

    # Initialize depth_counts to keep track of how many choices have a particular depth
    depth_counts = []
    prev_depth = 0
    for path in sorted_loaa_choices:
        depth = len(path)
        if depth != prev_depth:
    ...
    # Create the attention mask for loaa
    loaa_attn_mask = torch.eye(loaa_len, loaa_len)
    loaa_attn_mask[:, 0] = 1
    start = 0
    for i in range(len(depth_counts)):
        for j in range(depth_counts[i]):
            cur_loaa_choice = sorted_loaa_choices[start + j]
            # retrieve ancestor position
            if len(cur_loaa_choice) == 1:
                continue
            ancestor_idx = []
            for c in range(len(cur_loaa_choice) - 1):
                ancestor_idx.append(sorted_loaa_choices.index(cur_loaa_choice[:c+1]) + 1)
            loaa_attn_mask[j + start + 1, ancestor_idx] = 1
        start += depth_counts[i]

    # Generate tree indices for the loaa structure
    loaa_tree_indices = torch.zeros(loaa_len, dtype=torch.long)
    loaa_tree_indices[0] = 0
    start = 0
    for i in range(len(depth_counts)):
        for j in range(depth_counts[i]):
            cur_loaa_choice = sorted_loaa_choices[start + j]
            loaa_tree_indices[start + j + 1] = cur_loaa_choice[-1] + TOPK * i + 1
        start += depth_counts[i]

    # Generate position IDs for the loaa structure
    loaa_position_ids = torch.zeros(loaa_len, dtype=torch.long)
    start = 0
    for i in range(len(depth_counts)):
        loaa_position_ids[start + 1: start + depth_counts[i] + 1] = i + 1
        start += depth_counts[i]

    # Generate retrieval indices for loaa structure verification
    retrieve_indices_nest = []
    retrieve_paths = []
    for i in range(len(sorted_loaa_choices)):
        cur_loaa_choice = sorted_loaa_choices[-i-1]
        retrieve_indice = []
        if cur_loaa_choice in retrieve_paths:
            continue
        else:
            for c in range(len(cur_loaa_choice)):
                retrieve_indice.append(sorted_loaa_choices.index(cur_loaa_choice[:c+1]))
                retrieve_paths.append(cur_loaa_choice[:c+1])
    ...
    loaa_buffers = {
        "loaa_attn_mask": loaa_attn_mask.unsqueeze(0).unsqueeze(0),
        "tree_indices": loaa_tree_indices,
        "loaa_position_ids": loaa_position_ids,
    ...
    loaa_buffers = {
        ...
        for k, v in loaa_buffers.items()
    }
    return loaa_buffers


def initialize_loaa(input_ids, model, loaa_attn_mask, past_key_values):
    """
    Initializes the loaa structure for a given model.

    This function performs the following operations:
    1. Forward pass through the model to obtain the loaa logits, original model outputs, and logits.
    2. Sets the loaa attention mask within the base model.

    Args:
    - input_ids (torch.Tensor): The input tensor containing token ids.
    - model (loaaLMHead): The model containing the loaa layers and base model.
    - loaa_attn_mask (torch.Tensor): The attention mask designed specifically for the loaa structure.
    - past_key_values (list of torch.Tensor): Contains past hidden states and past attention values.

    Returns:
    - loaa_logits (torch.Tensor): Logits from the loaa heads.
    ...
    model.base_model.model.loaa_mask = loaa_attn_mask
    return loaa_logits, logits
...
def reset_loaa_mode(
    ...
    """
    Resets the loaa settings and the past key-values to their initial state.

    This function ensures that after any operations involving loaa,
    the base model and its settings return to their default state.
    Specifically, it performs the following tasks:
    1. Clears the loaa attention mask in the base model.
    2. Resets the loaa mode in the base model.
    ...
    - None
    """
    model.base_model.model.loaa_mask = None
    model.base_model.model.loaa_mode = None
...
def generate_candidates(loaa_logits, logits, tree_indices, retrieve_indices, temperature = 0, posterior_threshold=0.3, posterior_alpha = 0.09, top_p=0.8, sampling = 'typical', fast = False):
    """
    Generate candidates based on provided logits and indices.
    
    Parameters:
    - loaa_logits (torch.Tensor): Logits from a specialized loaa structure, aiding in candidate selection.
    ...
    - tuple (torch.Tensor, torch.Tensor): A tuple containing two sets of candidates:
        1. Cartesian candidates derived from the combined original and loaa logits.
    ...
    # Extract the TOPK candidates from the loaa logits.
    candidates_loaa_logits = torch.topk(loaa_logits[:, 0, -1], TOPK, dim = -1).indices

    # Combine the selected candidate from the original logits with the topk loaa logits.
    candidates = torch.cat([candidates_logit, candidates_loaa_logits.view(-1)], dim=-1)
...
def tree_decoding(
    ...
    - loaa_position_ids (torch.Tensor): Positional IDs associated with the loaa structure.
    - input_ids (torch.Tensor): Input sequence IDs.
    - retrieve_indices (list or torch.Tensor): Indices for reordering the logits.
    
    Returns:
    - tuple: Returns loaa logits, regular logits, and other outputs from the model.
    """

    # Compute new position IDs by adding the loaa position IDs to the length of the input sequence.
    position_ids = loaa_position_ids + input_ids.shape[1]

    # Use the model to decode the tree candidates. 
    # The model is expected to return logits for the loaa structure, original logits, and possibly other outputs.
    tree_loaa_logits, outputs, tree_logits = model(
        tree_candidates,
        output_orig=True,
    ...
    loaa_logits = tree_loaa_logits[:, 0, retrieve_indices]
    return loaa_logits, logits, outputs
...
def update_inference_inputs(
    ...
    - outputs, logits, loaa_logits (torch.Tensor): Model's outputs from the previous inference step.
    ...
    - loaa_logits (torch.Tensor): Updated loaa logits.
