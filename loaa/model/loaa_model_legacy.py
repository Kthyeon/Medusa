...
from .loaa_choices import mc_sim_7b_63
...
class LoaaModel(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            loaa_num_heads (int, optional): Number of additional tokens to predict. Defaults to 3.
            loaa_num_layers (int, optional): Number of ResBlock layers for each Loaa head. Defaults to 0.
        """
        super().__init__()
        ...
        self.medusa_head = nn.ModuleList(
            [
                nn.Sequential(
                    LoaBlock(self.hidden_size, self.config.loaa_width),
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        """
        Args:
            loaa_head_name_or_path (str): Name or path of the Loaa head to load.
            **kwargs: Additional keyword arguments for loading the base model.

        Returns:
            LoaaModel: A LoaaModel instance loaded from the given path.
        """
        loaa_config = LoaaConfig.from_pretrained(loaa_head_name_or_path)
        if loaa_num_heads is not None:
            print("Overriding loaa_num_heads as:", loaa_num_heads)
            loaa_config.loaa_num_heads = loaa_num_heads
        if base_model is not None:
            print("Overriding base_model as:", base_model)
            loaa_config.base_model_name_or_path = base_model
            
        base_model = KVLlamaForCausalLM.from_pretrained(
            loaa_config.base_model_name_or_path, **kwargs
        )

        model = cls(
            base_model,
            loaa_config.loaa_num_heads,
            loaa_config.loaa_num_layers,
            loaa_config.base_model_name_or_path,
        )
        loaa_head_path = os.path.join(loaa_head_name_or_path, "loaa_lm_head.pt")
        if os.path.exists(loaa_head_path):
            filename = loaa_head_path
        else:
            filename = hf_hub_download(loaa_head_name_or_path, "loaa_lm_head.pt")
        loaa_head_state_dict = torch.load(filename, map_location=base_model.device)
