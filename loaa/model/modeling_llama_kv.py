...
# 2. Modifying attention mask for integration with loaa
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def _prepare_decoder_attention_mask(self, attention_mask, input_shape, inputs_embeds, past_key_values_length):
        ...
        # [MODIFIED] add loaa mask
        if hasattr(self, "loaa_mask") and self.loaa_mask is not None:
            loaa_mask = self.loaa_mask
            loaa_len = loaa_mask.size(-1)
            combined_attention_mask[:, :, -loaa_len:, -loaa_len:][
                loaa_mask == 0
            ] = combined_attention_mask.min()
            if hasattr(self, "loaa_mode"):
                # debug mode
                if self.loaa_mode == "debug":
                    torch.save(combined_attention_mask, "loaa_mask.pt")
